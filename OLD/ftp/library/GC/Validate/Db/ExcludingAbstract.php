 ...
 *
 */
abstract class GC_Validate_Db_ExcludingAbstract extends Zend_Validate_Abstract
{
    /**
     * Error constants
     */
     ...
     */
    protected $_messageTemplates = array(self::ERROR_NO_RECORD_FOUND => 'No other record where "%field1%" is "%value%" was found',
                                         self::ERROR_RECORD_FOUND    => 'Another record where "%field1%" is "%value%" was found'
                                         );
    /**
     * @var array
     */
     ...
     */
    public function __construct($table, $field1, $excludeField, Zend_Db_Adapter_Abstract $adapter = null)
    {
        if ($adapter !== null) {
        ...
        $this->_excludeField   = (string) $excludeField;//will be excluded
    }
        /**
    ...
    {
        parent::_setValue($value[$this->_field1 ]);
    }
    /**
    ...
    protected function _query(array $value)
    {
        ...
        $select->from($this->_table)
               ->columns(array($this->_field1,$this->_excludeField))
               ->where($this->_adapter->quoteIdentifier($this->_field1).' = ?', $value[$this->_field1])

               ->where($this->_adapter->quoteIdentifier($this->_excludeField).' != ?', $value[$this->_excludeField]);
