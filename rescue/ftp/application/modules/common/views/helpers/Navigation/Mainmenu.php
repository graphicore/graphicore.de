...
//copied from Zend_View_Helper_Navigation_Menu
//i changed the rendering functions (i.e. htmlify and _renderMenu )to meet my needs
class Common_View_Helper_Navigation_Mainmenu
    extends Zend_View_Helper_Navigation_HelperAbstract
{
    ...
    public function htmlify(Zend_Navigation_Page $page)
    {
        ...
        return sprintf(
            '<%1$s%2$s>%3$s</%1$s>',//if image substitution is planned we need that span '<%1$s%2$s><span>%3$s</span></%1$s>',
            $element,
                                   ...
                                   $onlyActive)
    {
        ...
        foreach ($iterator as $page) {
            ...
            if ($depth > $prevDepth) {
                // start new ul tag
                if ($ulClass && $depth ==  0) {
                    $ulClass = sprintf(' class="%1$s"',$ulClass);
                } else {
                    $ulClass = '';
                }
                $html[] = sprintf('<ul%1$s>',$ulClass);
            ...
            $classes = ($page->liClass)? array($page->liClass) : array();
            if($isActive)
            {
                $classes[] = 'active';
            }
            // render li tag and page
            $liClass = (count($classes)) ? ' class="'.join(' ',$classes).'"' : '';
            $html[] = sprintf
            (
                '<li%1$s>%2$s',
                $liClass,
