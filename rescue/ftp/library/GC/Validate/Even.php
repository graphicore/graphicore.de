 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class GC_Validate_Even extends Zend_Validate_Abstract
{

    const NOT_ALLOWED = 'notEven';

    /**
     * @var array
     */
    protected $_messageTemplates = array(
        self::NOT_ALLOWED => "'%value%' is neither even nor in the allowed list: %allowed%"
    );
    ...
    public function setAllowed($allowed = NULL)
    {
        ...
        if(!is_array($allowed) || empty($allowed)){
            throw new Exception('$allowed must be numeric or array');
        }
    ...
    public function isValid($value)
    {
        ...
        $return = (in_array($value, $this->_allowed) || (0 === $value % 2));
        if (!$return) {
            $this->_error();
        }
        return $return;
